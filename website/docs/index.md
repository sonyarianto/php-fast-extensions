...
hero:
  ...
  tagline: "Native Zend API bindings via ext-php-rs. Streaming, constant-memory CSV and XLSX readers — 9.8x faster than fgetcsv, ~50x faster than a PhpSpreadsheet full load, and ~2-4 MB peak memory on multi-hundred-MB files."
  actions:
    - theme: brand
      text: CsvStreamer
      link: /csv-streamer/
...
features:
  ...
  - icon: 🧠
    ...
    details: Lazy, row-by-row streaming with a single reused buffer. Peak memory stays at 2-4 MB on a 349 MB CSV or a 231 MB workbook. No memory_limit errors, ever.
  - icon: 🗂️
