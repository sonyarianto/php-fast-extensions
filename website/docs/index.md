...
hero:
  ...
  tagline: "Native Zend API bindings via ext-php-rs. Streaming, constant-memory CSV, XLSX and JSON readers — 9.8x faster than fgetcsv, ~50x faster than a PhpSpreadsheet full load, ~1,000x less memory than json_decode on huge arrays, and ~2-4 MB peak memory on multi-hundred-MB files."
  actions:
    - theme: brand
      text: CsvStreamer
      link: /csv-streamer/
...
features:
  ...
  - icon: 🧠
    ...
    details: Lazy, row-by-row streaming with a single reused buffer. Peak memory stays at 2-4 MB on a 349 MB CSV, a 231 MB workbook, or a 226 MB JSON array — while json_decode needs ~2.1 GB and OOMs at the default limit.
  - icon: 🗂️
    title: Native PHP ergonomics
    details: Implements Iterator, so foreach just works. Associative rows keyed by header, native cell types (int, float, bool, datetime, null), custom delimiters.
  - icon: 🦀
    title: Built on Rust
